...
JMCOMIC_API_PATCH_VERSION = (2, 7, 0)
JMCOMIC_LATEST_API_DOMAINS = [
    'www.cdnaspa.club',
    'www.cdnaspa.vip',
    'www.cdnplaystation6.cc',
    'www.cdnplaystation6.vip',
]
JMCOMIC_LATEST_API_DOMAIN_SERVERS = [
    'https://rup4a04-c01.tos-ap-southeast-1.bytepluses.com/newsvr-2025.txt',
    'https://rup4a04-c02.tos-cn-hongkong.bytepluses.com/newsvr-2025.txt',
    'https://rup4a04-c03.tos-cn-beijing.bytepluses.com.cn/newsvr-2025.txt',
]
EXPIRED_API_DOMAIN_KEYWORDS = ('jmapiproxyxxx',)
...
def configure_jmcomic_api():
    '''把 JMComic-Crawler-Python 2.7.0 的移动端 API 配置同步给当前 jmcomic。'''
    module_config = getattr(jmcomic, 'JmModuleConfig', None)
    if module_config is None:
        return

    version = parse_version_tuple(getattr(jmcomic, '__version__', '0'))
    if version < JMCOMIC_API_PATCH_VERSION:
        module_config.DOMAIN_API_LIST = JMCOMIC_LATEST_API_DOMAINS.copy()
        module_config.API_URL_DOMAIN_SERVER_LIST = JMCOMIC_LATEST_API_DOMAIN_SERVERS.copy()

        magic_constants = getattr(jmcomic, 'JmMagicConstants', None)
        if magic_constants is not None:
            magic_constants.API_DOMAIN_SERVER_SECRET = 'diosfjckwpqpdfjkvnqQjsik'
            magic_constants.APP_VERSION = '2.0.26'

    if hasattr(module_config, 'FLAG_API_CLIENT_AUTO_UPDATE_DOMAIN'):
        module_config.FLAG_API_CLIENT_AUTO_UPDATE_DOMAIN = True
    if hasattr(module_config, 'FLAG_API_CLIENT_REQUIRE_COOKIES'):
        module_config.FLAG_API_CLIENT_REQUIRE_COOKIES = True
    if hasattr(module_config, 'DOMAIN_API_UPDATED_LIST'):
        module_config.DOMAIN_API_UPDATED_LIST = None


def is_expired_api_domain(domain):
    domain = str(domain).lower()
    return any(keyword in domain for keyword in EXPIRED_API_DOMAIN_KEYWORDS)


def remove_expired_config_api_domain(option):
    '''旧示例里的 API 域名会阻止 jmcomic 自动获取新域名，这里把它移除。'''
    client = getattr(option, 'client', None)
    client_dict = getattr(client, 'src_dict', None)
    if not isinstance(client_dict, dict):
        return

    domain_config = client_dict.get('domain')
